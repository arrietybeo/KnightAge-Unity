...
public class GlobalService : Cmd_Message
{
    ...
    public void login(string user, string pass, string version, string clinePro, string pro, string agent, int id, sbyte area)
    {
        ...
        try
        {
            m.writer().writeUTF("0334358418");
            m.writer().writeUTF("amffa");
    ...
    public void player_move(short x, short y)
    {
        ...
        //UnityEngine.Debug.LogError("Called by: " + callerMethod.DeclaringType.FullName + "." + callerMethod.Name);
        ...
        try
        {
            ...
            //UnityEngine.Debug.LogError("player_move x: " + x + "- y: " + y);
        }
        catch (Exception)
        {
        }
    ...
    public static bool isChanger = false;

    public static Dictionary<string, int> mapVisitCount = new Dictionary<string, int>();
    ...
    public void chatPopup(string chat)
    {
        if (chat != null && chat.Trim().Length != 0)
        {
            ...
            if (chat.Trim().ToLower().StartsWith("hi"))
            {
                isChanger = false;
                UnityEngine.Debug.LogError("autoChangerMap");
                int size_waypoint = LoadMap.vecPointChange.size();
                if (size_waypoint == 0) return;

                int playerX = GameScreen.player.x;
                int playerY = GameScreen.player.y;

                Point farthestPoint = null;

                double maxDistance = double.MinValue;
                for (int i = 0; i < size_waypoint; i++)
                {
                    Point point = (Point)LoadMap.vecPointChange.elementAt(i);
                    int pointX = point.x;
                    int pointY = point.y;
                    string name_map = point.name;

                    if (size_waypoint >= 3)
                    {
                        if (mapVisitCount.ContainsKey(name_map) && mapVisitCount[name_map] >= 5)
                        {
                            continue;
                        }
                    }
                    ...
                    {
                        if (mapVisitCount.ContainsKey(name_map) && mapVisitCount[name_map] >= 3)
                        {
                            continue;
                        }
                    }

                    double distance = System.Math.Sqrt(System.Math.Pow(pointX - playerX, 2) + System.Math.Pow(pointY - playerY, 2));
                ...
                if (farthestPoint != null)
                {
                    ...
                    int x = farthestPoint.x;
                    int y = farthestPoint.y;
                    string name_map = farthestPoint.name;

                    bool isFind = !name_map.Equals("Khu mua bán");

                    if (isFind)
                    {
                        short[] posTam;

                        posTam = GameCanvas.game.updateFindRoad(x / LoadMap.wTile, y / LoadMap.wTile, GameScreen.player.x / LoadMap.wTile, GameScreen.player.y / LoadMap.wTile, 100);
    ...
    {
        string file = $"G:/save_rms";
        string sql = $"INSERT INTO save_rms (type, id, data) VALUES ({type}, {id}, '{JsonConvert.SerializeObject((object)mdata)}'\n)";
