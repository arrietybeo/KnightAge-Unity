...
public class LoadMap
{
    ...
    public void loadmap(int id, sbyte[] mbyte)
    {
        try
        {
            ...
            string filepath = "G://LOG_SQL//map_data.sql";
            ...
            //Debug.LogError("mapW: " + mapW + "- mapH: " + mapH);
            for (int i = 0; i < mapW * mapH; i++)
            {
                sbyte b = dataInputStream.readByte();

                //string sql = "INSERT INTO map_data (id_map, mapW, mapH, tile, map_paint) VALUES (" + id + ", " + mapW + ", " + mapH + ", " + num + ", '" + b + "');\n";
            ...
            string sql = "INSERT INTO map_data (id_map, mapW, mapH, tile, map_paint) VALUES (" + id + ", " + mapW + ", " + mapH + ", " + num + ", '" + JsonConvert.SerializeObject((object)mapPaint) + "');\n";
            File.AppendAllText(filepath, sql);
    ...
    public void load_ItemMap(int id_map, sbyte[] mbyte)
    {
        try
        {
            string filepath = "G://LOG_SQL//map_elements.sql";
            string sql = "";
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                //Debug.LogError("ID: " + id + " - " + mainItemMap.IDItem + " - " + mainItemMap.IDImage + " - " + mainItemMap.dx + " - " + mainItemMap.dy + " - " + mainItemMap.Block.Length);
                ...
                sql += $"INSERT INTO map_elements (id, id_map, x, y) VALUES ({id}, {id_map}, {num3}, {num4});\n";
            ...
            if (dataInputStream.available() > 0 && !GameCanvas.lowGraphic)
            {
                ...
                for (int j = 0; j < num5; j++)
                {
                    ...
                    string[] array = mFont.split(text2, ";");
                    short IDItem = short.Parse(array[0]);
                    short IDImage = short.Parse(array[1]);
                    int x = int.Parse(array[2]);
                    int y = int.Parse(array[3]);
                    int dx = int.Parse(array[4]);
                    int dy = int.Parse(array[5]);
                    int typeEffect = int.Parse(array[6]);
                    int valueEffect = int.Parse(array[7]);


                    string file = "G://LOG_SQL//map_effect.sql";

                    string sqle = $"INSERT INTO map_effect (id_map, id_item, id_image, x, y, dx, dy, size_eff, type_effect, value_effect) VALUES ({id_map}, {IDItem}, {IDImage}, {x}, {y}, {dx}, {dy}, {num5}, {typeEffect}, {valueEffect});\n";

                    File.AppendAllText(file, sqle);
