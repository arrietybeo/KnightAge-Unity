...
public class PaintInfoGameScreen
{
            ...
            if (GameCanvas.isPointerSelect)
            {
                ...
                for (int k = 0; k < LoadMap.vecPointChange.size(); k++)
                {
                    ...
                    if (GameCanvas.isPoint(num3 - 12, num4 - 12, 24, 24))
                    {
                        ...
                        Player.setXFocus(point.x);
                        Player.setYFocus(point.y);
                        Player.timeFocus = 9;
                        GameCanvas.isPointerSelect = false;
                        if (Player.isAutoFire == 1)
                        {
                ...
                if (mainObject != null)
                {
                    ...
                    if (GameCanvas.isPointerSelect)
                    {
                        ...
                        if (tile != -1 && tile != 1)
                        {
                            ...
                            timeMove = 3;
                            GameCanvas.isPointerSelect = false;
                            Player.setXFocus(num);
                            Player.setYFocus(num2);
                        }
                        else
                        {
                            posTam = null;
    ...
    public void paintPos_minimap(mGraphics g, int x, int y)
    {
        ...
        if (!isMapThachdau())
        {
            ...
            mFont.tahoma_7_yellow.drawString(g, "x" + GameScreen.player.x + ": y" + GameScreen.player.y, x, y, 1, mGraphics.isFalse);

            //if (GlobalService.isChanger)
            //{
            //    GlobalService.gI().chatPopup("hi");
            //    timeCounter = 0f;
            //    return;
            //}
            ...
            //    GlobalService.gI().chatPopup("hi");  // Thực hiện hành động
            //    timeCounter = 0f;  // Reset thời gian sau khi hoàn thành
            //}
